(()=>{
  ...
  // All old per-provider/per-project overrides become implementation details.
  ...
  // Inject simple client AI credentials into the existing Worker summary request.
  // The Worker v28 layer consumes this object and never returns the API key.
  ...
  function hideLegacyAiDeployment(dialog){
    ...
    if(serviceAi){
      const small=serviceAi.querySelector('small');
      if(small)small.textContent='AI 请求经你的 Worker 转发到所填写的 OpenAI-compatible URL。';
    }
  }
  ...
  function mount(){
    ...
    const navProvider=dialog.querySelector('[data-settings-jump="provider"]');
    if(navProvider)navProvider.hidden=true;
    const provider=dialog.querySelector('[data-settings-section="provider"]');
    if(provider)provider.hidden=true;

    aiSection.innerHTML=`
      <div class="simple-ai-v28-head">
        <div><span>AI</span><h3>AI 配置</h3></div>
        <i class="simple-ai-v28-pill" data-simple-ai-pill>未配置</i>
      </div>
    ...
    const urlInput=aiSection.querySelector('[data-simple-ai-url]');
    const apiInput=aiSection.querySelector('[data-simple-ai-api]');
    const nameInput=aiSection.querySelector('[data-simple-ai-name]');
    const pill=aiSection.querySelector('[data-simple-ai-pill]');
    const status=aiSection.querySelector('[data-simple-ai-status]');
    const reveal=aiSection.querySelector('[data-simple-ai-reveal]');

    function load(){
      const value=simple();
      urlInput.value=value.url||'';
      apiInput.value=value.api||'';
      nameInput.value=value.name||'';
      const ready=complete(value);
      pill.textContent=ready?'已配置':'未配置';
      pill.className=`simple-ai-v28-pill ${ready?'ok':''}`;
    }
    function read(){return{url:clean(urlInput.value).replace(/\/+$/,''),api:clean(apiInput.value),name:clean(nameInput.value)}}
    function save(show=true){
      const value=read();
      saveJson(SIMPLE_KEY,value);
      applyRuntime(value);
      const ready=complete(value);
      pill.textContent=ready?'已配置':'未配置';
      pill.className=`simple-ai-v28-pill ${ready?'ok':''}`;
      if(show){
        status.textContent=ready?'✓ 已保存。下一次搜索直接使用这套 AI 配置。':'请把 URL、API、Name 三项填完整。';
        status.className=`simple-ai-v28-status ${ready?'ok':'fail'}`;
      }
      return value;
    }
    async function test(){
      const value=save(false);
      const worker=clean(localStorage.getItem(WORKER_KEY)||'').replace(/\/+$/,'');
      if(!complete(value)){
        status.textContent='请先把 URL、API、Name 三项填完整。';status.className='simple-ai-v28-status fail';return;
      }
      if(!worker){
        status.textContent='AI 三项已填写，但系统还需要“服务地址”中的 Worker URL 来转发请求。';status.className='simple-ai-v28-status fail';return;
      }
      status.textContent='正在测试…';status.className='simple-ai-v28-status';
      try{
        const response=await nativeFetch(`${worker}/api/ai/summaries`,{
          method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},
          body:JSON.stringify({style:'brief',ai:value,items:[{key:'test:1',type:'test',title:'三字段 AI 配置连接测试',source:'local test',year:new Date().getFullYear(),authors:[],text:'只需要返回一句简短中文摘要，用于确认接口可以正常调用。'}]})
        });
        const data=await response.json().catch(()=>({}));
        if(!response.ok)throw new Error(data.error||`HTTP ${response.status}`);
        status.textContent=`✓ 连接成功 · ${data.model||value.name}`;
        status.className='simple-ai-v28-status ok';
        pill.textContent='可用';pill.className='simple-ai-v28-pill ok';
      }catch(error){
        status.textContent=`连接失败：${error.message}`;
        status.className='simple-ai-v28-status fail';
        pill.textContent='失败';pill.className='simple-ai-v28-pill fail';
      }
    }
    ...
    reveal.addEventListener('click',()=>{
      const show=apiInput.type==='password';apiInput.type=show?'text':'password';reveal.textContent=show?'隐藏':'显示';
    });
