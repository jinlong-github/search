(() => {
  ...
  const DEFAULTS = {enabled:false,style:'standard',batchSize:10,onlyWithSource:true};
  let generation = 0;
  ...
  function sourceFor(item) {
    return clean(item.venue || item.domain || item.source || item.assignees?.[0] || '');
  }
  function sourceText(item) {
    return clean(item.abstract || item.description || item.snippet || '');
  }
  ...
  function cacheFingerprint(item,style) { return `${clean(item.title)}|${sourceText(item).slice(0,280)}|${style}`; }
  ...
  function setUiStatus(text,stateName='') {
    document.querySelectorAll('[data-ai-runtime-status]').forEach(node => {
  ...
  async function enhance() {
    ...
    try {
      ...
      summaries.forEach(entry => {
        ...
        if (item) saved[key] = {fingerprint:cacheFingerprint(item,config.style),summary,at:Date.now()};
      });
  ...
  if (basePerformSearch) {
    performSearch = async function performSearchWithAi(rawQuery) {
      const result = await basePerformSearch(rawQuery);
      schedule(180);
      return result;
    };
  }
  ...
  status.className = 'ai-runtime-status';
  status.dataset.aiRuntimeStatus = '';
  status.textContent = '';
